...
use super::*;
use super::IPreviewedFile::IPreviewedFile;
...
pub fn buildPreviewedFile(file:&str)->Result<Box<dyn IPreviewedFile>,String>{
    ...
    if is_image_file(file).is_ok(){
        return Ok(Box::new(PreviewedFile_Image::PreviewedFile_Image::new(file)));
    }
    else{
        return Ok(Box::new(PreviewedFile_Unknown::PreviewedFile_Unknown::new(file)));
    }
